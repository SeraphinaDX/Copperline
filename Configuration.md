...
| `log_dir` | string | `"~/.local/state/copperline/logs"` | Root directory for IRC logs. `~` and `~/...` are expanded. |
...
The current configuration does not have a global `logging = false` switch or per-channel log controls. Changing `log_dir` changes the destination but does not disable logging.
